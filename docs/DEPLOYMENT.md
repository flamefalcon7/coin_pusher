...
|---------|------|-------------|
| shared | (internal) | Builds shared TS types (watch mode) |
| client | :5173 | Vite dev server (HMR) |
| backend | :4000, :4010 | Go API + WebSocket gateway + debug endpoints |
...
You should see 8 services: `nginx`, `nats`, `postgres`, `backend`, `executor`, `indexer`, `prometheus`, `grafana`.
...
| `WALLET_SEED` | `BACKEND_WALLET_SEED` | **Hex-encoded master seed** (at least 32 bytes / 64 hex chars, optional `0x` prefix). Generate with `openssl rand -hex 32`. Derives ALL on-chain private keys: deposit addresses, hot wallet (index 999999) for withdrawals, and sweeper keys. This is the highest-sensitivity secret. |
...
The single hex seed derives all on-chain keys via BIP-32/BIP-44 (`m/44'/60'/0'/0/<index>`):
