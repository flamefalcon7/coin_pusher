...
import { KEY_COIN_CONFIG, COIN_CONFIG } from "@coin-pusher/shared";
...
export class KeyCoin {
  ...
  private static readonly CCD_DISABLE_VEL_SQ =
    KEY_COIN_CONFIG.CCD_DISABLE_VELOCITY ** 2;
  ...
  constructor(
    ...
    const coreHalfHeight =
      KEY_COIN_CONFIG.THICKNESS / 2 - KEY_COIN_CONFIG.BORDER_RADIUS;
    const coreRadius = KEY_COIN_CONFIG.RADIUS - KEY_COIN_CONFIG.BORDER_RADIUS;
    ...
    const colliderDesc = RAPIER.ColliderDesc.roundCylinder(
      ...
      .setRestitution(KEY_COIN_CONFIG.RESTITUTION);
  ...
  update(): void {
    ...
    // Sleep check
    if (vSq < KeyCoin.LIN_THRESHOLD_SQ && wSq < KeyCoin.ANG_THRESHOLD_SQ) {
